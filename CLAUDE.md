...
13. **Refined Prompt Convention (`/refine` and `/plan`).** See `.agents/standards/refined-prompt-convention.md` for the full standard. In brief:
    - **When using `/refine`:** Always auto-save to `.agents/refined-prompts/YYYY-MM-DD_NN-<slug>.md` with YAML frontmatter: `refined: true`, `refined_at`, `refined_by: Claude`. No user confirmation needed.
