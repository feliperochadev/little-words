...
4. **Commit** — build the commit message from the collected unshipped entries:
   - **Subject line**: titles from every unshipped entry (e.g. `[tag] Description`), joined with ` / `, followed by ` (apsc)`. **Strip the `**` bold markers.** Trim to 72 characters if needed.
