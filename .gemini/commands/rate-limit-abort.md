# /rate-limit-abort — Safe abort when approaching usage quota (Gemini)

Call this command when you detect you are approaching 95% of your available quota mid-task. It reverts all uncommitted work, persists the task context, marks Gemini as temporarily unavailable, and exits cleanly so another agent can resume.
...
4. **Mark Gemini as unavailable** — update `.agents/agent-config.json`:
...
5. **Output the following message and stop:**
   ...
   Gemini marked as unavailable in .agents/agent-config.json

   Another agent (Claude or Codex) can resume by running /check-unfinished-tasks.
   Gemini will re-mark itself available on next invocation.
   ```
